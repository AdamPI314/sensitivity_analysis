...
def bar_1D_SI(file_dir):    
    """
    bar
    """
    colors, markers, _ = get_colors_markers_linestyles()
    
    SI_1st_data = np.loadtxt(os.path.join(file_dir, "output", "SI_1st.csv"), delimiter=',', dtype=float)
    ...
    bar1 = ax.bar(x, SI_1st_data, width, color=colors, bottom=0, align="center")
    ...
    ax.set_title("$1^{st}$" + " order SI" + " of ignition delay time wrt. initial concentration")
    ax.text(1, 1.0 - 0.1 ,"SUM($1^{st}$ order SI): " + "%1.4g" % (np.sum(SI_1st_data)))
